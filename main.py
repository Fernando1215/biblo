    ...
    <script>
        ...
        async function login() {
            ...
            try {
                ...
                // Elementos de Admin
                const adminElements = document.querySelectorAll('#tituloAgregarLibro, #panelUsuario > .input-group, #panelUsuario > textarea, #panelUsuario > button.btn-success');
                
                if (userRole === 'admin') {
                    adminElements.forEach(el => el.style.display = (el.tagName === 'DIV' ? 'flex' : 'block'));
        ...
        async function cargarCatalogo() {
            try {
                ...
                libros.forEach(libro => {
                    ...
                    if (userRole !== null) {
                        botonesHTML += '<button class="btn-small btn-success" onclick="agregarAMiBiblioteca(' + libro.id + ')">Agregar</button>';
                    }
                    
                    botonesHTML += '<button class="btn-small btn-primary" onclick="verDetalle(' + libro.id + ')">Ver</button>';
                    
                    if (userRole === 'admin') {
                        botonesHTML += ' <button class="btn-small btn-warning" onclick="editarLibro(' + libro.id + ')">Editar</button>';
                        botonesHTML += ' <button class="btn-small btn-danger" onclick="eliminarLibro(' + libro.id + ')">Eliminar</button>';
                    }
